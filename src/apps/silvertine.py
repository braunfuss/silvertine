...
try:
    ...
    print('Pyrocko is required for silvertine!'
...
silvertine is a probabilistic earthquake source inversion framework.
...
def command_detect(args):
    ...
    def setup(parser):
        ...
        parser.add_option(
            ...
            help='end')
        parser.add_option(
            '--lf', dest='lf', type=float, default=2,
            help='end')
...
'''.format(c=Color,
    ...
    parser, options, args = cl_parse(
        'init', args, setup,
        'Use silvertine init list to show available examples.')
    ...
    if args[0].startswith('example_'):
        if len(args) == 1:
            ...
            logger.info('Hint: To create a project, use: silvertine init <example> '
