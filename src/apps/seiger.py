...
try:
    ...
    print('Pyrocko is required for seiger!'
          'Go to https://pyrocko.org/ for installation instructions.')


logger = logging.getLogger('seiger.main')
km = 1e3
...
subcommand_descriptions = {
    ...
    'diff': 'compare two configs or other normalized seiger YAML files',
    'qc-polarization': 'check sensor orientations with polarization analysis',
    'upgrade-config': 'upgrade config file to the latest version of seiger',
    'version': 'print version number of seiger and its main dependencies',
}
...
from seiger.setup_info import version as __version__
usage_tdata['version_number'] =  __version__


usage = '''%(program_name)s <subcommand> [options] [--] <arguments> ...

seiger is a probabilistic earthquake source inversion framework.

This is seiger version %(version_number)s.
...
def command_locate(args):
    ...
    if options.show is not False:
        options.show = True
    result, best_model = seiger.locate.locate1D.solve(scenario_folder=project_dir, show=options.show, n_tests=options.nevents, scenario=option.scenario, data_folder=options.data_folder)
...
def command_scenario(args):
    ...
    from seiger import scenario as seiger_scenario
    project_dir = args[0]
    scenario = seiger.scenario.seigerScenario(
...
def command_init(args):
    ...
    from .cmd_init import seigerInit

    seiger_init = seigerInit()
    ...
    help_text = '''Available configuration examples for seiger.

{c.BOLD}Example Projects{c.END}

    Deploy a full project structure into a directory.

    usage: seiger init <example> <projectdir>
...
{c.BOLD}Config Sections{c.END}
    ...
    usage: seiger init <section>

    where <section> is any of the following:

{sections_list}
'''.format(c=Color,
           examples_list=print_section(seiger_init.get_examples()),
           sections_list=print_section(seiger_init.get_sections()))
    ...
    parser, options, args = cl_parse(
        'init', args, setup,
        'Use seiger init list to show available examples.')
    ...
    if args[0].startswith('example_'):
        if len(args) == 1:
            config = seiger_init.get_content_example(args[0])
            if not config:
                help_and_die(parser, 'Unknown example: %s' % args[0])

            sys.stdout.write(config+'\n\n')

            logger.info('Hint: To create a project, use: seiger init <example> '
            ...
            try:
                seiger_init.init_example(args[0], args[1], force=options.force)
            except OSError as e:
                print(str(e))

    else:
        sec = seiger_init.get_content_snippet(args[0])
        if not sec:
...
def command_init_old(args):
    ...
    except seiger.seigerError as e:
        die(str(e))
...
def command_events(args):
    ...
    config_path = args[0]
    try:
        config = seiger.read_config(config_path)

        for event_name in seiger.get_event_names(config):
            print(event_name)
...
def command_check(args):
    ...
    from seiger.environment import Environment
...
def command_cluster(args):
    from seiger import Clustering
    from seiger.clustering import metrics, methods, read_config, write_config
    ...
    try:
        parser, options, args = cl_parse(
            ...
            details='Available clustering methods: [%s]. Use '
                    '"seiger cluster <method> --help" to get list of method '
        ...
        if options.write_config:
            ...
            seiger.cluster(run_path, clustering, metric=options.metric)
...
def make_report(env_args, event_name, conf, update_without_plotting, nthreads):
    from seiger.environment import Environment
    from seiger.report import report
    try:
...
def command_report(args):
    ...
    if options.serve or options.serve_external:
        ...
        if not entries_generated and not options.index_only:
            logger.info('Nothing to do, see: seiger report --help')
...
def command_diff(args):
    ...
    from seiger.config import diff_configs
    diff_configs(*args)
...
def command_version(args):
    def setup(parser):
        parser.add_option(
            ...
            help='only print seiger\'s version number')
        parser.add_option(
    ...
    if options.short:
        print(seiger.__version__)
        return

    elif not options.failsafe:
        from seiger import info
        print(info.version_info())
        return

    print("seiger: %s" % seiger.__version__)
