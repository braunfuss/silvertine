...
class fzone(object):                        #FEHM zone object.
    ...
    def fix_temperature(self,T,multiplier=1.e10,file=None):
        ...
        '''
        if not self._parent:
            print('fix_temperature() only available if zone associated with fdata() object')
            return
    ...
    def fix_pressure(self,P, T=30., impedance=1.e6, file = None):
        ...
        '''
        if not self._parent:
            print('fix_pressure() only available if zone associated with fdata() object')
            return
    ...
    def copy_from(self,from_zone=None,grid_new=None,method = 'nearest',grid_old=None):
        ...
        if from_zone.type == 'rect':         # if rectangular zone, copy across bounding box
            ...
            if not from_zone.grid or not from_zone.nodelist:
                print('Supplied zone does not contain grid information.')
                return
    ...
    def topo(self,save='',cbar=True,equal_axes=True, method = 'nearest', divisions=[30,30], xlims=[],
        ...
        *Example:*
        ...
        if not self.nodelist:
            print('No node information, aborting...')
            return
        if not title:
    ...
    def _set_nodes(self,value):
        if self.type == 'rect':
            print('Error: nodelist for zone defined by content of points.')
            return
    ...
    def _set_points(self,value):
        if self.type in ['list','nnum']:
            print('Error: points defined by content of nodelist.')
            return
        self._points = value
    ...
    def _set_permeability(self,value):
        ...
        # set commands
        if not self._parent:
            print('Zone not associated with input file, no macro changes made.')
            return
    ...
    def _check(self):
        ...
        if self.type == None:
            print('WARNING: Zone '+str(self.index)+' type not assigned.')
        if self.points == None:
            print('WARNING: Zone '+str(self.index)+' points array is empty.')
class fmacro(object):                         #FEHM macro object
    ...
    def _set_subtype(self,value):
        self._subtype = value
        if self.type != 'stressboun':
            print('WARNING: subtype ignored for non-stressboun macros')
    ...
    def _check(self):
        ...
        for key in self.param.keys():
            if self.param[key] == None:
                print('WARNING: Macro '+str(self.type)+':'+str(self.zone.index)+' '+key+' not assigned.')
    def _get_info(self):
...
class fincon(object):                         #FEHM restart object.
    ...
    def stressgrad(self,xgrad,ygrad,zgrad,xygrad = 0.,xzgrad=0.,yzgrad=0.,calculate_vertical=False,vertical_fraction=False):
        ...
        if not self.filename:
            print('ERROR: initial conditions file containing temperature/pressure data not loaded.')
            return
        if not self._parent._associate:
            print('ERROR: incon file not associated with parent data file - node and coordinate data not available.')
            return
...
class fstrs(object):                        #FEHM stress module.
    ...
    def _get_info(self):
        if self.param['ISTRS']:
            print('Stress module activated.')
        else:
            print('Stress module inactive')
        return
...
class fcarb(object):                        #FEHM CO2 module.
    ...
    def _get_info(self):
        if self.iprtype != 1:
            print('CO2 module activated.')
        else:
            print('CO2 module inactive')
            return
        prntStr = 'Components:'
...
class _relperm(object):                         # private class for relative permeability model
    ...
    def _set_type(self,value):
        ...
        if value not in rlpm_dicts.keys():
            return
        self._type = value
...
class files(object):                        #FEHM file constructor.
    ...
    def write(self):
        ...
        if self.co2_inj_time:
            if self._parent.carb.iprtype == 1:
                print("WARNING: CO2 injection flag requested but no carb macro specified, CO2 injection flag will be ignored.")
            else:
                outfile.write('999\n')
...
#        self._hist = fhist()
    ...
    def _write_hist(self,outfile):                                #Writes HIST macro.
        if not self.hist.nodelist and not self.hist.zonelist and not self.hist.zoneflux:
            return
#        if self.hist.nodelist and self.hist.zonelist: print 'WARNING: not set up to print both hist nodes and hist zones'; return
    ...
    def temperature_gradient(self,filename,offset=0.,first_zone = 100,auxiliary_file=None,hydrostatic = 0):
        ...
        if not os.path.isfile(filename):
            print('ERROR: cannot find temperature gradient file \''+filename+'\'.')
            return
        ...
        if not commaFlag and not spaceFlag:
            print('ERROR: incorrect formatting for \''+filename+'\'. Expect first column depth (m) and second column temperature (degC), either comma or space separated.')
            return
    ...
    def _add_macro(self,macro,overwrite=False):                    #Adds the macro to data file
        ...
        if isinstance(macro.zone,list) and len(macro.zone)==0: macro.zone = self.zone[0]     # assign everywhere zone
            ...
            if macro.zone in self.zone.keys(): macro.zone = self.zone[macro.zone]
                ...
                print('ERROR: Specified zone '+str(ind)+' for macro '+macro.type+' does not exist.')
