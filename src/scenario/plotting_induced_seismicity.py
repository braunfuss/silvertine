...
def get_OKWO_eqs():
    te, me = [], []
    t0 = datetime.strptime('20090101', '%Y%m%d')           # reference time 1 Jan 19
...
def draw_eq_pressure_dist(fig, ax, x, test_data="oklahoma"):

    #### load variables
    if test_data == "oklahoma":
        labels = [
            'calibrated models',
    ...
    for j in range(4):
        ...
        ax.fill_between(tj[j], rts_poisson_05[ts[j]], rts_poisson_95[ts[j]],
                        color=colors_loc[j], alpha=.5)     # plot Poisson enveloppe
    ...
    if test_data == "oklahoma":
        ...
        te, me = get_eqs(test_data)            # read list of EQ
        tbins = np.linspace(2012,2018,25)   # bins for observed seismicity rate
    ...
    ax.set_xlabel('time',fontsize=text_size)
    ax.set_ylim(0, 50)
    ax.set_ylabel('Monthly number of earthquakes ('+r'$M \geq 3$)',
                  fontsize=text_size)
    ax.legend(fontsize=text_size, loc='upper right')
