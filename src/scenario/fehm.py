# based on Supplementary material to Dempsey and Riffault (2018) "Response of induced seismicity to injection rate reduction: models of delay, decay, quiescence, recovery and Oklahoma"
# Authors
# -------
...
from silvertine.scenario.inducedseismicity import InducedSeismicity
...
# -----------------------
	# convert units
t = t/365.25+2012			# decimal years
r = r/1.e3					# km
ti = np.linspace(2012,2025,1001)   	# interpolation times
n = 0.*ti
	# loop over layers and compute seismicity contribution of each

import pickle
import numpy as num
f = open("kuperkoch_data", 'rb')
times, pressure, temp, rate = pickle.load(f)
f.close()
min_time = num.min(times)
iterator = -1

pressures_day = []
days = []
sum_pressure = 0
for k, time in enumerate(times[0:15500]):
	time_diff_day = int((time - min_time)/86400)
	if iterator == -1:
		iterator = time_diff_day+1
		days.append(iterator)
	if time_diff_day == iterator:
		sum_pressure= sum_pressure+pressure[k]
	if iterator != time_diff_day:
		pressures_day.append(sum_pressure)
		iterator = iterator+1
		days.append(iterator)

		sum_pressure = 0

for p in pressures_day:
	# format FEHM data
	p = [p] * len(r)
	sim = {'r':num.asarray(r),'t':num.asarray(t),'p':num.asarray(p)}
	# create object
...
times = np.genfromtxt('events_fem.txt', usecols=0, delimiter=',')[::-1]
