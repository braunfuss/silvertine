...
from .meta import Path, HasPaths, silvertineError
...
def read_config(path):
    try:
        ...
        raise silvertineError(
            'Cannot read silvertine configuration file: %s' % path)

    if not isinstance(config, Config):
        raise silvertineError('Invalid silvertine configuration in file "%s".' % path)
...
def write_config(config, path):
    try:
        ...
        guts.dump(
            config,
            filename=path,
            header='silvertine configuration file, version %s' % __version__)

        config.change_basepath(basepath)

    except OSError:
        raise silvertineError(
            'Cannot write silvertine configuration file: %s' % path)
...
class YPathError(silvertineError):
    pass
